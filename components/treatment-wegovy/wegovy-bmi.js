...
const WegovyBmi = () => {
    ...
    const [weightLoss20Percent, setWeightLoss20Percent] = useState(null);
    ...
    const calculateBbmi = (data) => {
        ...
        if (!heightM || !weightUnit) return null;
        return +(weightkg / (heightM * heightM)).toFixed(2);
    }

    const onSubmit = (data) => {
        if (!data) return;
        const bmiValue = calculateBbmi(data);
        setBmi(bmiValue);

        const currentWeightKg = data.weightKg;
        const weightLoss = +(currentWeightKg * 0.2).toFixed(2);
        setWeightLoss20Percent(weightLoss);
    ...
    return (
        <section className='mt-5 py-5 md:px-12.5 bg-white rounded '>
            <Container>
                <div className='flex flex-col md:flex-row items-center gap-6'>
                    ...
                    <div className="bg-[#ecdfff] p-10 rounded-[20px] flex flex-col gap-5 flex-[1.2] order-2 md:order-1">
                        ...
                        {showBmi && (
                            <div className="bg-white rounded-2xl p-8 flex flex-col gap-5">
                                ...
                                <h2 className="text-3xl font-semibold text-[#160647]">
                                ...
                                <div className=" w-full relative grid grid-cols-[repeat(5,1fr)]">
                                    ...
                                    <div
                                        className="absolute h-7 w-7 -top-1.5 bg-white rounded-full border-3 border-[#8d69c9]"
                                        style={{ left: `${bmi}%` }}
                                    >
                                    </div>
                                </div>
                                ...
                                <div className="py-8 rounded-2xl bg-[#ccb8ebe1] flex flex-col items-center justify-center gap-5">
                                    <p className="text-[#160647] text-lg">
                                        Your estimated weight loss in 1 year
                                    </p>
                                    <div className="flex flex-row gap-5">
                                        <p className="text-4xl font-semibold">
                                            <span className="text-lg text-[#6b5b95]">▼</span> {weightLoss20Percent}<sup className="text-lg">kg</sup>
                                        </p>
                    ...
                    <div className="flex flex-col items-start gap-3 flex-1 order-1 md:order-2">
